      ...
      if (canPlayMoan())
      {
        ...
        if (ArousalMode.val == "Interactive")
        {
          ...
          if (lastCollisionDiff <= minLastCollisionTime)
          {
            ...
            if (lastInteractionTime != -1.0f)
            { // First of all, only if we already interacted with her
              if (normalizedArousal >= 1.0f && normalizedArousal < 4.0f && lastInteractionDiff > 9.0f)
              { // Now, we only bump up the arousal if we're under the maximum and if we let the girl rest for 9secs
